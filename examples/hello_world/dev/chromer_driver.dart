...
class ChromeDriver {
  ...
  Future<void> start() async {
    // TODO(nurhan): improvement check if exists.
    // Install Chrome Driver.
    final bool isInstalled = await install();
    if(isInstalled) {
  ...
  Future<bool> install() async {
    exitingFile = io.File(path.join(driverDir.path, 'chromedriver'));
    if (!exitingFile.existsSync()) {
  ...
  Future<bool> _installDriver() async {
    ...
    try {
      ...
      throw Exception('Failed to download chrome driver $chromeDriverVersion. $e');
    } finally {
      client.close();
    }
